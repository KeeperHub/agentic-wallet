...
import type { HookDecision } from "../../src/types.js";
...
describe("Hook integration -- JSON envelope for Claude Code PreToolUse", () => {
  ...
  it("ask tier emits permissionDecision:ask so Claude Code prompts inline (v0.1.4 collapse)", async () => {
    // v0.1.4 removed the server-approval branch; all amounts between auto
    // and block resolve to inline ask.
    const hook = await createPreToolUseHook({
      configLoader: () => Promise.resolve(testSafety),
    });
    const decision = await hook({
    ...
    const env = toEnvelope(decision);
    expect(env.hookSpecificOutput.permissionDecision).toBe("ask");
  });
});
